...
正如标题所示，这是一篇 work-in-progress 的文章，也就意味有很多地方需要补充的地方，以及还没有来得及很详细地核对。请在阅读本文的时候保持怀疑。

这篇文章我主要来谈谈 Linux 中各种内存指标之间的关系，比如说什么是页缓存、什么是匿名缓存、什么是共享内存等等，这些各种内存之间的包含与被包含关系（我下面画了一个图）。
